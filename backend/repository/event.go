...
func (r *Repository) GetEvents() ([]model.Event, error) {
	...
	if err := r.db.Preload("Participants").Preload("Participants.User").Find(&events).Error; err != nil {
		return nil, err
	}
...
func (r *Repository) GetEventByID(id uint) (*model.Event, error) {
	...
	if err := r.db.Preload("Participants").Preload("Participants.User").First(&event, id).Error; err != nil {
		return nil, err
	}
